...
export const UserProfile = () => {
  ...
  useEffect(() => {
    if (id) {
      api.get(`/announcements/user/${id}`).then((resp) => setUserAnnouncements(resp.data));
    }
  }, []);
  ...
  return (
    <>
      ...
      <Container>
        <Banner>
          <Avatar username={userAnnouncements[0]?.user.name || ''} variant="big" />
          <div>
        ...
        {announcements.length > 0 && <ShowCase listAnnouncements={userAnnouncements} />}
      </Container>
      <Footer />
    </>
  );
